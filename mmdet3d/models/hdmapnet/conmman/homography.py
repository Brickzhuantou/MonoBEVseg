...
def perspective(cam_coords, proj_mat, h, w, extrinsic, post_rts, offset=None):
    """
    P = proj_mat @ (x, y, z, 1)
    Project cam2pixel
    ...
    if extrinsic:
        pix_coords[:, 0] += offset[0] / 5 *3
        pix_coords[:, 2]
        pix_coords = torch.stack([pix_coords[:, 2], pix_coords[:, 0]], axis=1)
    else:
        pix_coords = pix_coords[:, :2, :] / (pix_coords[:, 2, :][:, None, :] + eps)
        if post_rts != None:
            # pix_coords = post_rts @ pix_coords
            post_rts = post_rts.reshape(-1, 4, 4)
            pix_coords = post_rts[:,:2,:2] @ pix_coords
            pix_coords = pix_coords + post_rts[:,:2,3].unsqueeze(2).repeat(1,1,pix_coords.shape[2])
...
def ipm_from_parameters(image, xyz, K, RT, target_h, target_w, extrinsic, post_RT=None):
    ...
    #     P = post_RT @ P
    P = P.reshape(-1, 4, 4)
    pixel_coords = perspective(xyz, P, target_h, target_w, extrinsic, post_RT, image.shape[1:3])
