...
class ViewTransformation(BaseModule):
    ...
    def __init__(self, fv_size, bv_size, n_views=6):
        ...
        self.con2d = nn.Conv2d(256, 64, kernel_size=3, padding=1)
...
class HDMapNet_Neck(BaseModule):
    def __init__(self, data_config, grid_config, downsample=16, **kwargs):
        super(HDMapNet_Neck, self).__init__()
        ...
        res_x = bv_size[1] * 3 // 5
        ipm_ybound = [-res_x, res_x, 0.3]
        ipm_xbound = [0, res_x*2, 0.3]
        self.ipm = IPM(ipm_xbound, ipm_ybound, N=1, C=64, extrinsic=True)
    ...
    def get_Ks_RTs_and_post_RTs(self, cam2imgs, sensor2ego, post_rots, post_trans):
        ...
        # Ks[:,:,:3,:3] = cam2imgs # hdmapnet内参变换用mlp学习
