...
@HEADS.register_module()
class Center_with_mapseg_Head(BaseModule):
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(Center_with_mapseg_Head, self).__init__(init_cfg=init_cfg)
        ...
        self.maphead = builder.build_head(cfg_map)
        ...
        self.map_feature_cropper = BevFeatureSlicer(
            grid_config, map_grid_config
        )
    ...
    def forward_single(self, x):
        ...
        """
        x_map = self.map_feature_cropper(x).unsqueeze(0)
        pred_map = self.maphead(x_map)
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, semantic_indices, preds_dicts, **kwargs):
        ...
        # map相关的loss
        pred_segmap = preds_dicts[1][0]['semantic_seg']
        target_segmap = semantic_indices
        
        loss_segmap = self.maphead.semantic_seg_criterion(
            pred_segmap.unsqueeze(dim=1).float(),
            target_segmap.unsqueeze(dim=1).long(),
        )
        
        loss_dict[f'task0.loss_segmantic'] = 2*loss_segmap
        
        # loss_dict['loss'] += 2*loss_segmap
        
        
        return loss_dict

    def get_map_indices(self, map_pred_dicts, img_metas):
        """得到预测语义栅格图的具体语义形式用于计算IoU

        Args:
            pred_dicts (_type_): _description_
            img_metas (_type_): _description_
        """
        
        res = self.maphead.get_semantic_indices(
                map_pred_dicts[0]['semantic_seg'],
            )
        return res
...
class Mapseg_Head(BaseModule):
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(Mapseg_Head, self).__init__(init_cfg=init_cfg)
    ...
    def loss(self, semantic_indices, preds_dicts, **kwargs):
        
        loss_dict = dict()
        loss_dict['loss'] = 0
        # map相关的loss
        pred_segmap = preds_dicts[0][0]['semantic_seg']
        target_segmap = semantic_indices
    ...
    def get_map_indices(self, map_pred_dicts, img_metas):
        ...
        res = self.maphead.get_semantic_indices(
                map_pred_dicts[0][0]['semantic_seg'],
            )
        return res
