...
class bevformer_encoder(TransformerLayerSequence):
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if lid == 0 and self.return_intermediate:
                output = layer(
                bev_query,
                key,
                value,
                ...
                **kwargs)
                ...
                if lid == 1:
                    if semnatic_prior is not None:
                        semnatic_prior = semnatic_prior.flatten(1)
                        bev_query = bev_query + semnatic_prior[:,:,None] # 第二层开始给query引入语义先验
                output = layer(
                    bev_query,
                    key,
                    value,
