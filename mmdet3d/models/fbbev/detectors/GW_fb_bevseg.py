...
class GWFBSeg(CenterPoint):
    ...
    def __init__(self, 
                  ...
                  **kwargs):
        super(GWFBSeg, self).__init__(**kwargs)
        ...
        self.road_occ_head = builder.build_head(road_occ_head) if road_occ_head else None
    ...
    def extract_img_bev_feat(self, img, img_metas, semantic_indices, **kwargs):
        ...
        if self.with_specific_component('forward_projection'):
            bev_feat = self.forward_projection(cam_params, context, depth, **kwargs) # (n,64,200,200,8)
            return_map['cam_params'] = cam_params
        else:
            bev_feat = None
            
        # DONE:路面重建获取ground mask
        if self.with_specific_component('road_occ_head'):
            # road_mask_logit = self.road_occ_head(bev_feat)
            road_mask_logit = self.road_occ_head(bev_feat.detach()) # 不想让road_occ_head的梯度传到backbone
            
            road_mask = road_mask_logit.sigmoid() > self.road_occ_head.mask_thre
            
            # if road_mask.requires_grad: # during training phase
                # gt_bev_mask = kwargs['gt_bev_mask'].to(torch.bool)
                # bev_mask = gt_bev_mask | bev_mask
            return_map['road_mask_logit'] = road_mask_logit
        else:
            road_mask = None
        
        if self.with_specific_component('frpn'): # not used in FB-OCC
            bev_mask_logit = self.frpn(bev_feat.mean(-1))
            
            # 二值化任务
            if self.frpn.binary_cls:
                bev_mask = bev_mask_logit.sigmoid() > self.frpn.mask_thre
                
                if bev_mask.requires_grad: # during training phase
                    gt_bev_mask = semantic_indices.to(torch.bool)
                    bev_mask = gt_bev_mask | bev_mask
                return_map['bev_mask_logit'] = bev_mask_logit    
            else:
                # 先得到二值化mask作为bevmask
                pred_semantic_scores = bev_mask_logit.float(
                ).sigmoid()
                pred_semantic_scores, pred_semantic_indices = torch.max(
                    pred_semantic_scores, dim=1)
                background_mask = pred_semantic_scores < self.frpn.semantic_thresh
                pred_semantic_indices[background_mask] = 0
                pred_semantic_indices = pred_semantic_indices.long()
                bev_mask = pred_semantic_indices.to(torch.bool)
                # 预测结果存下用于计算loss
                return_map['bev_mask_logit'] = bev_mask_logit
            
            # torch.save(bev_mask, './tmp/bev_mask.pt')
        else:
            bev_mask = None

        # DONE:根据路面估计结果进行back_projection
        if self.with_specific_component('backward_projection'):
            ...
            if bev_feat == None:
                ...
                if self.frpn==None or self.frpn.binary_cls:
                    semnatic_prior = None
                else:
                    pred_semantic_logits = bev_mask_logit.clone()
                    pred_semantic_indices = torch.argmax(pred_semantic_logits, dim=1)
                    semnatic_prior = pred_semantic_indices.float().sigmoid() # 作为语义先验
            ...
            if self.backward_projection.transformer.encoder.return_intermediate==True:
                ...
                for i in range(len(bev_feat_refined)):
                    if self.readd:
                        ...
                        bev_feat_final = self.bev_encoder(bevfeat_add)
                        return_map['img_bev_feat_{}'.format(i)] = bev_feat_final
                    else:
                ...
                if self.readd:
                    bev_feat_refined = (bev_feat_refined[..., None] + bev_feat).mean(-1)
    ...
    def extract_feat(self, points, img, img_metas, semantic_indices, **kwargs):
        """Extract features from images and points."""
        results={}
        if img is not None and self.with_specific_component('image_encoder'):
            img = self.prepare_inputs(img)
            results.update(self.extract_img_bev_feat(img, img_metas, semantic_indices, **kwargs))
    ...
    def forward_train(self,
        ...
        results= self.extract_feat(
            points, img=img_inputs, img_metas=img_metas, semantic_indices=semantic_indices, **kwargs)
        losses = dict()
        ...
        if self.with_specific_component('frpn'):
            if self.frpn.binary_cls:
                losses_mask = self.frpn.get_bev_mask_loss(semantic_indices.to(torch.bool), results['bev_mask_logit'])
                losses.update(losses_mask)
            else:
                losses_mask = self.frpn.get_bev_mask_loss(semantic_indices, results['bev_mask_logit'])
                losses.update(losses_mask)
        ...
        if self.with_specific_component('road_occ_head'):
            losses_road = self.road_occ_head.get_road_mask_loss(kwargs['gt_occupancy'], results['road_mask_logit'])
            losses.update(losses_road)
    ...
    def simple_test(self,
        ...
        if self.backward_projection and self.backward_projection.transformer.encoder.return_intermediate==True:
            pred_map_indices = self.simple_test_pts(results['img_bev_feat_1'], img_metas, rescale=rescale)
        else:
            pred_map_indices = self.simple_test_pts(results['img_bev_feat'], img_metas, rescale=rescale)
