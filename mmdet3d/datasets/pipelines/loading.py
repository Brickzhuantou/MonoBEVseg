    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam_name in cam_names:
            ...
            if self.use_seg:
                ...
                seg_folder_path = '/home/chen_jj/datasets/nuscenes/CAM_FRONT_SEG/'
                filename = os.path.join(seg_folder_path, base_filename)
...
class LoadAnnotationsBEVDepth(object):
    ...
    def __init__(self, bda_aug_conf, classes, is_train=True, mono=False):
    ...
    def mono_filter(self, gt_boxes, gt_labels, results):
        # 读取相机内外参
        
        if gt_boxes.shape[0] > 0:
            cam_data = results['curr']['cams']['CAM_FRONT']
            intrin = torch.Tensor(cam_data['cam_intrinsic']) # 读取内参
            fu, cu = intrin[0, 0].numpy(), intrin[0, 2].numpy()
            sensor2ego, ego2global = \
                self.get_sensor_transforms(results['curr'], 'CAM_FRONT')
                
            rots = sensor2ego[:3, :3]
            trans = sensor2ego[:3, -1]
            
            # 把gt box的坐标转到像素平面进行筛选
            # 把自车坐标系的点转到相机坐标系
            for i in range(3):
                gt_boxes[:, i] = gt_boxes[:, i] - np.array(trans)[i]
            gt_boxes[:, :3] = torch.from_numpy(np.dot(rots.T, gt_boxes[:, :3].transpose(1,0)).transpose(1,0))
            
            # 然后012表示左右（右为正），上下，前后（前为正）
            uc = gt_boxes[:,0]/gt_boxes[:,2] *fu +cu
            
            index = (gt_boxes[:,2]>0) & (uc<1600) & (uc>0) 
            gt_boxes = gt_boxes[index==True] # 要保留index下的点
            gt_labels = gt_labels[index==True]
            
            # 将坐标再转回车体坐标系；
            gt_boxes[:, :3] = torch.from_numpy(np.dot(rots, gt_boxes[:, :3].transpose(1,0)).transpose(1,0))
            for i in range(3):
                gt_boxes[:, i] = gt_boxes[:, i] + np.array(trans)[i]
    ...
    def __call__(self, results):
        ...
        if self.mono:
            gt_boxes,gt_labels = self.mono_filter(gt_boxes, gt_labels, results)
...
class RasterizeMapVectors(object):
    ...
    def __call__(self, results):
        vectors = results['vector']
        
        # 进行vector的数据增强处理（与box同步）
        bda_mat = results['img_inputs'][-1].to(torch.double)
        
        for vector in vectors:
            vector['pts'] = (bda_mat@torch.tensor(vector['pts']).unsqueeze(-1)).squeeze(-1).numpy()
...
@PIPELINES.register_module()
class Mono_RasterizeMapVectors(object):
    """Load multi channel images from a list of separate channel files.
    ...
    def __call__(self, results):
        ...
        split = semantic_masks.shape[2]//2
        
        semantic_masks = semantic_masks[:,:,split:]
        semantic_indices = semantic_indices[:,split:]        
...
class LoadOccupancy(object):
    ...
    def __init__(self,
                    occ_grid_config,
                    map_grid_config, 
                    occupancy_path='/home/chen_jj/workspace/zhujt_workspace/gts',
                    num_classes=17,
        ...
        self.map_grid_config = map_grid_config
    ...
    def __call__(self, results):
        ...
        if self.ignore_nonvisible:
            occupancy[~visible_mask.to(torch.bool)] = 0
        
        occupancy[occupancy!=11] = 0
        occupancy[occupancy==11] = 1 # 路面标签为1
        ...
        # sample for map config
        map_bev_resolution, map_bev_start_position, map_bev_dimension = calculate_birds_eye_view_parameters(
                self.map_grid_config['x'], self.map_grid_config['y'], self.map_grid_config['z'],)
        map_x = torch.arange(
                        map_bev_start_position[0], self.map_grid_config['x'][1], map_bev_resolution[0])
        map_y = torch.arange(
                        map_bev_start_position[1], self.map_grid_config['y'][1], map_bev_resolution[1])
        bev_end_position = torch.tensor(
                        [row[1] - row[2] / 2.0 for row in [self.occ_grid_config['x'], self.occ_grid_config['y'], self.occ_grid_config['z']]])
        norm_range_x = bev_end_position[0]
        norm_range_y = bev_end_position[1]

        norm_map_x = map_x / norm_range_x
        norm_map_y = map_y / norm_range_y

        map_grid = torch.stack(torch.meshgrid(norm_map_y, norm_map_x), dim=2)
        occ_mask_for_sample = occupancy.unsqueeze(0).permute(0, 3, 2, 1).float()
        sample_map_mask = F.grid_sample(occ_mask_for_sample, map_grid.unsqueeze(0), mode='nearest', align_corners=True)
        occupancy = sample_map_mask.squeeze(0).permute(1, 2, 0)
