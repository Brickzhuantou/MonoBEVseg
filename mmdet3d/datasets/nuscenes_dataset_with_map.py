...
from .map_utils import VectorizedLocalMap, preprocess_map
from nuscenes import NuScenes
from nuscenes.eval.detection.evaluate import NuScenesEval
from plugins.metrics.evaluate import DetectionEval_mono

@DATASETS.register_module()
class NuScenesDataset_with_map(Custom3DDataset):
    r"""NuScenes Dataset.
    ...
    def __init__(self,
        ...
        # map相关
        self.map_grid_config = map_grid_conf
        self.map_data_root = data_root
        map_xbound, map_ybound = self.map_grid_config['xbound'], self.map_grid_config['ybound']
        ...
        self.vector_map = VectorizedLocalMap(
            dataroot=self.map_data_root,
            patch_size=self.map_patch_size,
            canvas_size=self.map_canvas_size,
        )
        self.mono_gt_test = mono_gt_test
        # if self.mono_gt_test:
        #     self.CLASSES = ('car', 'bus', 'pedestrian')
    ...
    def get_map_ann_info(self, info):
        ...
        vectors = self.vector_map.gen_vectorized_samples(
            info['map_location'], info['ego2global_translation'], info['ego2global_rotation'])
    ...
    def get_data_info(self, index):
        ...
        input_dict['vector'] = self.get_map_ann_info(info)
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            pred_map = det['pred_map_indices'].cpu().numpy()
            gt_map = det['gt_map_indices'].numpy()
            map_annos = list()
            
            map_annos.append(pred_map)
            map_annos.append(gt_map)
            ...
            if sample_token in nusc_map_annos:
                nusc_map_annos[sample_token].extend(map_annos)
            else:
    ...
    def _evaluate_single(self,
        ...
        if self.mono_gt_test == True:
            
            # self.eval_detection_configs.class_names = ['car', 'bus', 'pedestrain']
            
            nusc_eval = DetectionEval_mono(
                nusc,
    ...
    def _format_map(self, results, jsonfile_prefix=None):
        """Convert the results to the standard format.
    ...
    def format_results_map(self, results, jsonfile_prefix=None):
        """Format the results to json (standard format for COCO evaluation).
    ...
    def evaluate_map(self,
        ...
        Returns:
            dict[str, float]: Results of each evaluation metric.
        """
        result_files, tmp_dir = self.format_results_map(results, jsonfile_prefix)
