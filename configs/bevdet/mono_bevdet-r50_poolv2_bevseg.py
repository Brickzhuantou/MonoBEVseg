...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        bbox_coder=dict(
            ...
            post_center_range=[0, -61.2, -10.0, 61.2, 61.2, 10.0],
            max_num=500,
            score_threshold=0.1,
            out_size_factor=8,
    ...
    test_cfg=dict(
        pts=dict(
            pc_range=point_cloud_range[:2],
            post_center_limit_range=[0, -61.2, -10.0, 61.2, 61.2, 10.0],
            max_per_img=500,
            max_pool_nms=False,
...
optimizer = dict(type='AdamW', lr=2e-4, 
                #  paramwise_cfg=dict(
                #     custom_keys={
                #         'SeparateHead': dict(lr_mult=0.1),   # 降低检测头的学习率；
                #     }),
                 weight_decay=1e-07
                 )
