...
def add_map_adj_info(extra_tag):
    nuscenes_version = 'v1.0-trainval'
    dataroot = './data/nuscenes/'
    nuscenes = NuScenes(nuscenes_version, dataroot)
    ...
    for set in ['train', 'val']:
        ...
        for id in range(len(dataset['infos'])):
            ...
            # get sweep adjacent frame info
            sample = nuscenes.get('sample', info['token'])
            map_location = nuscenes.get('log', nuscenes.get('scene', sample['scene_token'])['log_token'])['location']
        ...
        with open('./data/nuscenes/%s_infos_%s_addmap.pkl' % (extra_tag, set),
                  'wb') as fid:
            pickle.dump(dataset, fid)

def add_map_adj_info2(extra_tag):
    nuscenes_version = 'v1.0-trainval'
    dataroot = './data/nuscenes/'
    nuscenes = NuScenes(nuscenes_version, dataroot)
    # nusc_can_bus = NuScenesCanBus(dataroot=can_bus_root_path)
    ...
    for set in ['train', 'val', 'test']:
        dataset = pickle.load(
            open('./data/nuscenes/%s_geo_%s.pkl' % (extra_tag, set), 'rb'))
        for id in range(len(dataset['infos'])):
            if id % 10 == 0:
                print('%d/%d' % (id, len(dataset['infos'])))
            info = dataset['infos'][id]
        ...
        with open('./data/nuscenes/%s2_geo_%s.pkl' % (extra_tag, set),
                  'wb') as fid:
            pickle.dump(dataset, fid)
