...
def parse_args():
    ...
    parser.add_argument(
        ...
        default = '/data1/users/zhujt_workspace/BEVDet-dev2.1/work_dirs/bevdet-r50_poolv2_bevseg/vis/pts_bbox/results_nusc.json')
    parser.add_argument(
        '--show-range',
        type=int,
        default=50,
    ...
    parser.add_argument(
        '--save_path',
        type=str,
        default='/data1/users/zhujt_workspace/BEVDet-dev2.1/work_dirs/bevdet-r50_poolv2_bevseg/vis/',
    ...
    parser.add_argument(
        '--mono', type=bool, default=False, help='if mono')
...
def main():
    ...
    if args.format == 'video':
        ...
        vout = cv2.VideoWriter(
            ...
            args.fps, (int(1600 / scale_factor * 3 + 400),
    ...
    for cnt, infos in enumerate(
        ...
        # 读取地图文件
        map_res = res['map_results'][infos['token']]
        pred_segmap = np.array(map_res[0]) # (1,200,400)
        gt_segmap = np.array(map_res[1]) # (1,200,400)
        
        #改到这里        
        draw_pred_segmap = np.zeros([pred_segmap[0].shape[0], pred_segmap[0].shape[1], 3]) # 尺寸对齐
        draw_gt_segmap = np.zeros([pred_segmap[0].shape[0], pred_segmap[0].shape[1], 3])
        
        for ii in range(1,4):
            draw_pred_segmap[:,:,ii-1][pred_segmap[0]==ii] = 1
            draw_gt_segmap[:,:,ii-1][gt_segmap[0]==ii] = 1
            
        draw_pred_segmap = draw_pred_segmap.astype('uint8') * 255
        draw_gt_segmap = draw_gt_segmap.astype('uint8') * 255
        
        draw_pred_segmap = np.moveaxis(draw_pred_segmap, 0, 1)
        draw_gt_segmap = np.moveaxis(draw_gt_segmap, 0, 1)
        ...
        img = np.zeros((900 * 2 + canva_size * scale_factor, 1600 * 3 + 2 * 200 * scale_factor, 3),
                       dtype=np.uint8)
        img[:900, 200*scale_factor:200*scale_factor+1600 * 3, :] = np.concatenate(imgs[:3], axis=1)
        img_back = np.concatenate(
            [imgs[3][:, ::-1, :], imgs[4][:, ::-1, :], imgs[5][:, ::-1, :]],
            axis=1)
        img[900 + canva_size * scale_factor:, 200*scale_factor:200*scale_factor+1600*3, :] = img_back
        img = cv2.resize(img, (int(1600 / scale_factor * 3+2*200),
                               int(900 / scale_factor * 2 + canva_size)))
        ...
        img[int(900 / scale_factor)+300:int(900 / scale_factor) + 300 + map_x,
            :200, :] = draw_pred_segmap[::-1,:,:][:,::-1,:] # 转180°为了可视化
        
        img[int(900 / scale_factor)+300:int(900 / scale_factor) + 300 + map_x,
            -200:, :] = draw_gt_segmap[::-1,:,:][:,::-1,:]
